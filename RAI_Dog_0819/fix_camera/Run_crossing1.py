...
if __name__ == "__main__":
    ...
    #ser3 = armcontrol.check_serial('/dev/lidar',       115200, timeout=1)   # 原 ttyUSB1 (1-2.3) 右测距 避障使用
    #ser3 = armcontrol.check_serial('/dev/ttyCH343USB3',       115200, timeout=1) 
    ...
    if ser3 is None or ser2 is None or ser0 is None or ser1 is None or ser4 is None:
        print("[ERROR] Serial port connection failed. Please check the connections.")
        exit(1)


    #ser3 = serial.Serial('/dev/ttyCH343USB3', 115200, timeout=1)
    ser3 = serial.Serial('/dev/right', 115200, timeout=1)   # 原 ttyUSB1 (1-2.3) 右测距 避障使用
    #ser3 = serial.Serial('/dev/lidar', 115200, timeout=1)
    ...
    while True:  
        ...
        if actions.sm == 's_start':
            actions.f_start(img_processor,sport_client,armcontrol,ser3,ser4)
