...
import serial
from ArmControl_Nomark_for_linux import ArmControl
...
class Actions:
    ...
    def f_start(self,img_processor: Img_Processor,sport_client: SportClient, armcontrol: ArmControl):
    #def f_start(self,img_processor: Img_Processor,sport_client: SportClient):
        ...
        if self.micro_s == 0:
            ...
            timer.get_elapsed(10)
            self.right = read_distance1(ser)
            self.w_v_y_ = pid_start_dis_in.compute(self.right)
            print(f"right:{self.right}")
            sport_client.Move(0.2,self.w_v_y_,0)
            if timer.time_is_up:
                self.micro_s = 10
                timer.start_timer() 
    ...
    def f_in_circle(self,img_processor: Img_Processor,sport_client: SportClient,armcontrol: ArmControl,ser1,ser2):
