...
class RobustArmControl:
    ...
    def reset(self, *args, **kwargs):
        ...
        self._post_arm_operation(operation_type="reset", duration=operation_duration)
        return result
    ...
    def reset_arm(self, *args, **kwargs):
        ...
        operation_duration = time.time() - start_time
        self._post_arm_operation(operation_type="reset_arm", duration=operation_duration)
        return result
    ...
    def arm_put_fixed(self, *args, **kwargs):
        ...
        operation_duration = time.time() - start_time
        self._post_arm_operation(operation_type="put_fixed", duration=operation_duration)
        return result
    ...
    def arm_catch_and_put(self, *args, **kwargs):
        ...
        operation_duration = time.time() - start_time
        self._post_arm_operation(operation_type="catch_and_put", duration=operation_duration)
        return result
    
    def _post_arm_operation(self, operation_type="default", duration=0):
        """机械臂操作后的处理"""
        self.last_arm_operation_time = time.time()
        
        print(f"[INFO] {operation_type} operation completed in {duration:.1f}s, checking camera...")
        
        # 对于长时间操作，给摄像头一些恢复时间
        if operation_type == "catch_and_put" and duration > 30:
            print("[INFO] Long operation detected, giving camera time to stabilize...")
            time.sleep(2.0)  # 等待2秒让USB设备稳定
            
            # 刷新摄像头状态 - 重置时间戳以避免误判
            self.camera_manager.refresh_status()
        
        # 执行实际的摄像头测试而不是依赖状态检查
        try:
            print("[INFO] Testing camera functionality...")
            ret, frame = self.camera_manager.read_frame()
            if ret and frame is not None:
                print(f"[INFO] Camera test successful after {operation_type}")
                return  # 摄像头工作正常，直接返回
            else:
                print(f"[WARNING] Camera read failed after {operation_type}, attempting reconnection...")
        except Exception as e:
            print(f"[WARNING] Camera test exception after {operation_type}: {e}, attempting reconnection...")
        
        # 只有在实际读取失败时才重连
        print("[INFO] Attempting camera reconnection...")
        success = self.camera_manager.connect()
        if not success:
            print("[WARNING] Camera reconnection failed, will retry in main loop...")
        else:
            print("[INFO] Camera reconnection successful")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 初始化摄像头管理器
        print("[INFO] Initializing camera manager for crossing2...")
        ...
        if ser0 is None or ser1 is None or ser2 is None or ser3 is None:
        ...
        # 初始化机械臂 - crossing2特有的初始化序列
        print("[INFO] Initializing robotic arm for crossing2...")
        ...
        print("[INFO] Starting main control loop for crossing2...")
        frame_count = 0
