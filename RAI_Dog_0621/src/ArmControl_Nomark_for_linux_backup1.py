...
class ArmControl:
    ...
    def scan_range_for_circle(self, serial_port, pulse_start, pulse_end, pulse_step, use_time=200, delay=0.1):
        window = []
        pulse_window = []
        for pulse in range(pulse_start, pulse_end + 1, pulse_step):
            setBusServoPulse(1, pulse, use_time)
            d = read_distance(serial_port)
            print("The Distance is ",d)
            if d is not None and 300 < d < 450:
                print("OK")
                window.append(d)
                pulse_window.append(pulse)
                if len(window) == 10:
                    
                    min_idx = np.argmin(window)
                    best_dist = window[min_idx]
                    best_pulse = pulse_window[min_idx]
                    #print(f"pulse={best_pulse}, d={best_dist}")
                    return best_pulse, best_dist
            else:
                print("NO!!!")
                window = []
    ...
    def locate_target_angle(self,laser_port='/dev/ttyUSB0', baudrate=115200):
        ...
        if 200 <= fp <= 800:
            ...
            ser.close()
            return fp  
    def locate_target_angle_range_larger(self,laser_port='/dev/ttyUSB0', baudrate=115200):
        ser = serial.Serial(laser_port, baudrate, timeout=1)
        time.sleep(0.1)
        ...
        if 200 <= fp <= 800:
            ...
            ser.close()
            return fp  
    def locate_target_angle_for_circle(self,laser_port='/dev/ttyUSB0', baudrate=115200):
        ser = serial.Serial(laser_port, baudrate, timeout=1)
        time.sleep(0.1)
       
        fp, fd = self.scan_range_for_circle(ser, 350, 650, 5)
        if 300 <= fp <= 650:
            setBusServoPulse(1, fp, 500)
            ser.close()
            return fp  
    ...
    def locate_target_distance(self,laser_port='/dev/ttyUSB1', baudrate=115200):
        ...
        time.sleep(0.1)
        distances = []
        print("---Locate target distance---")
        for i in range(25):
            d = read_distance(ser)  
            print("Distance is ", d)
            if d is not None and d < 450 and d > 100 and i > 5 and i < 20:
    ...
    def control_arm(self, px, py, pz, nx, ny, th3_min=-120, th3_max=120, th3_step=1):
        ...
        while True:
            ...
            if (l2 - l3)**2 <= A**2 + B**2 and (l2 + l3)**2 >= A**2 + B**2:
                ...
                #P1= self.locate_target_angle_range_larger(laser_port='/dev/ttyUSB0', baudrate=115200)
    ...
    def arm_catch_and_put(self):
        ...
        px = (dist_mm + offset) / 1000.0
        py = 0.0
        pz = -0.03  # The vertical height difference from the center of the object to the center of servo 2
    ...
    def Arm_put_optimized(self):
        setBusServoPulse(2, 600, 200)
        time.sleep(1)
        self.reset_arm(400, 500, 500, 300, 530, 1000)
        # Step 1: Locate the target angle and distance
        P1= self.locate_target_angle_for_circle(laser_port='/dev/ttyUSB1', baudrate=115200)
        setBusServoPulse(1, P1, 1000)
        time.sleep(0.2)
