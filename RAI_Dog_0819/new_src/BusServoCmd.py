...
if platform.system() == "Windows":
        serial_port = "COM4"  # Windows下的串口
elif platform.system() == "Linux":
    
    serial_port = "/dev/ttyUSB2"  # Linux下的串口
    #serial_port = pick_port()
else:
    raise EnvironmentError("Unsupported platform")
serialHandle = serial.Serial(serial_port, 115200)  # 初始化串口， 波特率为115200
'''
def checksum(buf):
    # 计算校验和
    sum = 0x00
...
def serial_serro_wirte_cmd(ser2, id=None, w_cmd=None, dat1=None, dat2=None):
    '''
    写指令
    :param ser2: 串口对象
    :param id: 舵机ID
    ...
    try:
        ...
        # 如果写入失败，抛出异常让上层处理
        raise serial.SerialException(f"Serial write failed: {e}")

def serial_servo_read_cmd(id=None, r_cmd=None):
    '''
    发送读取命令
    注意：此函数需要全局变量 serialHandle，当前版本未使用
    :param id:
    :param r_cmd:
    :param dat:
    :return:
    '''
    # 此函数当前未使用，需要定义 serialHandle 才能正常工作
    print("[WARNING] serial_servo_read_cmd function is not implemented - serialHandle not defined")
    return None
    # buf = bytearray(b'\x55\x55')  # 帧头
    # buf.append(id)
    # buf.append(3)  # 指令长度
...
def serial_servo_get_rmsg(cmd):
    ...
    '''
    # 此函数当前未使用，需要定义 serialHandle 才能正常工作
    print("[WARNING] serial_servo_get_rmsg function is not implemented - serialHandle not defined")
    return None
