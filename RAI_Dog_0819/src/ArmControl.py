#ArmControl_test is for Robot Arm of Chen Shu
...
class ArmControl:
    ...
    def scan_range(self, serial_port, ser2, pulse_start, pulse_end, pulse_step, use_time=200, delay=0.1):
        ...
        for pulse in range(pulse_start, pulse_end + 1, pulse_step):
            ...
            if pulse == pulse_start or pulse == pulse_end:
                continue
            # 如果有有效读数，计算平均值
            if readings:
                filtered_d = np.mean(readings)
    ...
    def locate_target_angle(self, ser0, ser2, baudrate=921600):
        ...
        offsetpulse = 20
        if 200 <= fp <= 800:
            setBusServoPulse(1, fp - offsetpulse, 1000, ser2)
            time.sleep(0.15)    
            #ser.close()
            return fp - offsetpulse
    ...
    def control_arm(self, px, py, pz, ser2, nx = 1.0, ny = 0, th3_min=-120, th3_max=120, th3_step=1):
        ...
        if not np.isfinite(cos_val) or cos_val < -1.0 or cos_val > 1.0:
            ...
            time.sleep(1)
            setBusServoPulse(6, 700, 1000, ser2)
            time.sleep(0.5)
    ...
    def reset_arm(self, P1, P2, P3, P4, P5, P6, ser2):
        ...
        time.sleep(0.5)
        try:
            setBusServoPulse(6, int(P6), 1000, ser2)
        except (serial.SerialException, OSError) as e:
    ...
    def arm_catch_and_put(self, ser0, ser1, ser2):
        ...
        try:
            ...
            self.reset(350, 600, 0, 400, 500, ser2)
            time.sleep(0.1)
            ...
            print(f"[INFO] Located angle for servo 1: {P1}")
            time.sleep(0.2)
            ...
            py = 0.0
            pz = -0.07  # The vertical height difference from the center of the object to the center of servo 2
            ...
            P2, P3, P4 = self.Calculate_Arm_Angle(px+0.03, py, pz = -0.07)
            ...
            self.reset(165, 500, 100, 500, 500, ser2)
            time.sleep(0.2)
            try:
                setBusServoPulse(4, 330, 1000, ser2)
                time.sleep(0.5)
                setBusServoPulse(6, 200, 1000, ser2)
                time.sleep(0.5)
                setBusServoPulse(6, 0, 1000, ser2)
                time.sleep(0.8)
            ...
            self.reset(850, 500, 125, 300, 500, ser2)
            time.sleep(0.5)
            try:
                setBusServoPulse(2, 420, 1000, ser2)
                time.sleep(0.2)
                setBusServoPulse(4, 180, 1000, ser2)
                time.sleep(1)
                setBusServoPulse(6, 250, 1000, ser2)
                time.sleep(0.25)
                setBusServoPulse(6, 700, 1000, ser2)
                time.sleep(1)
                setBusServoPulse(2, 700, 1000, ser2)
                time.sleep(0.5)
            except (serial.SerialException, OSError) as e:
            ...
            #!!!!!!should get back to 125
            try:
                setBusServoPulse(1, 170, 1000, ser2)
                time.sleep(0.5)
                #setBusServoPulse(4, 220, 100, ser2)
                setBusServoPulse(4, 240, 1000, ser2)
                time.sleep(0.2)
                setBusServoPulse(3, 220, 1000, ser2)
                time.sleep(0.5)
    ...
    def arm_put_fixed(self, ser2):
        try:
            ...
            # setBusServoPulse(4, 500, 1000, ser2)
            # time.sleep(0.2)
            setBusServoPulse(1, 400, 1000, ser2)
            time.sleep(0.2)
            ...
            # time.sleep(1)
            #self.reset(165, 850, 0, 100, 500, ser2)
            setBusServoPulse(1, 165, 800, ser2)
            time.sleep(0.2)
            setBusServoPulse(2, 850, 800, ser2)
            time.sleep(0.2)
            setBusServoPulse(3, 0, 800, ser2)
            time.sleep(0.2)
            setBusServoPulse(4, 100, 800, ser2)
            time.sleep(0.2)
            ...
            try:
                ...
                if new_ser2.is_open:
                    ...
                    # 尝试完成关键的复位操作
                    try:
                        self.reset(165, 850, 0, 100, 500, new_ser2)
                        self.fin_crossing = True
...
if __name__ == "__main__":
    ser0 = serial.Serial('/dev/detector0', 921600, timeout=1)
    ser1 = serial.Serial('/dev/detector1', 115200, timeout=1)
    #ser3    = serial.Serial('/dev/detector2',   115200, timeout=1)   # serial=5959050042
    ...
    setBusServoPulse(2, 750, 1000, ser2)
    time.sleep(1)
