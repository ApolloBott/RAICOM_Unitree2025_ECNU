...
class RobustArmControl:
    ...
    def _post_arm_operation(self):
        ...
        if not self.camera_manager.is_camera_stable():
            ...
            # 暂停计时器
            from Timer import timer
            timer.print_timer_status("Before arm operation pause")
            timer.pause_timer()
            timer.print_timer_status("After arm operation pause")
            ...
            # 恢复计时器
            timer.print_timer_status("Before arm operation resume")
            timer.resume_timer()
            timer.print_timer_status("After arm operation resume")
        else:
            # 摄像头看起来正常，只做轻量级检查
            try:
                ...
                if not ret or frame is None:
                    ...
                    from Timer import timer
                    timer.print_timer_status("Before arm test pause")
                    timer.pause_timer()
                    self.camera_manager.connect()
                    timer.resume_timer()
                    timer.print_timer_status("After arm test resume")
            except Exception as e:
                print(f"[WARNING] Camera test failed: {e}, attempting reconnection...")
                from Timer import timer
                timer.print_timer_status("Before arm exception pause")
                timer.pause_timer()
                self.camera_manager.connect()
                timer.resume_timer()
                timer.print_timer_status("After arm exception resume")
    ...
    try:
        ...
        while True:  
            ...
            if debug_counter % 10 == 0:
                timer.print_timer_status(f"Main loop #{debug_counter}")
            ...
            if current_time - last_camera_check > 5.0:  # 每5秒检查一次
                if not camera_manager.is_camera_stable():
                    ...
                    timer.print_timer_status("Before stability check pause")
                    timer.pause_timer()  # 暂停计时器
                    timer.print_timer_status("During stability check pause")
                    camera_manager.connect()
                    timer.print_timer_status("Before stability check resume")
                    timer.resume_timer()  # 恢复计时器
                    timer.print_timer_status("After stability check resume")
            ...
            if not img_processor.finish_visual:
                ...
                if not ret or frame is None:
                    ...
                    timer.print_timer_status("Before read fail pause")
                    timer.pause_timer()  # 暂停计时器
                    timer.print_timer_status("During read fail pause")
                    time.sleep(0.1)  # 短暂等待后继续
                    timer.print_timer_status("Before read fail resume")
                    timer.resume_timer()  # 恢复计时器
                    timer.print_timer_status("After read fail resume")
                    continue
                
                frame_count += 1
                if frame_count % 30 == 0:  # 每30帧打印一次状态
                    print(f"[INFO] Processed {frame_count} frames, camera: {camera_manager.current_path}")
                    timer.print_timer_status(f"Frame #{frame_count}")
            ...
            # 在状态机处理前后打印计时器状态
            if debug_counter % 5 == 0:  # 每5个循环打印一次
                timer.print_timer_status(f"Before state machine - {actions.sm}")
            ...
            if debug_counter % 5 == 0:  # 每5个循环打印一次
                timer.print_timer_status(f"After state machine - {actions.sm}")   
